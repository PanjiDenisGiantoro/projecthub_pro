    ...
    <div class="pt-4 pb-6">
        <div class="flex items-center gap-1.5 text-xs text-gray-400 font-medium mb-5">
            <a href="{{ route('projects.index') }}"
                class="hover:text-blue-600 dark:hover:text-blue-400 transition">Projects</a>
            <span>&gt;</span>
            <a href="{{ route('projects.show', $ticket->project) }}"
                class="text-gray-600 dark:text-gray-300 font-semibold hover:text-blue-600 transition">{{ $ticket->project->name }}</a>
            <span>&gt;</span>
            <a href="{{ route('tickets.index', $ticket->project) }}"
                class="text-gray-600 dark:text-gray-300 font-semibold hover:text-blue-600 transition">Tickets</a>
            <span>&gt;</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    <div class="flex items-start justify-between gap-4 mb-4">
                        ...
                        <div class="flex gap-2 flex-shrink-0">
                            ...
                            <span
                                class="badge {{ $sc[$ticket->status] ?? '' }}">{{ ucfirst(str_replace('_', ' ', $ticket->status)) }}</span>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    ...
                    @if($user->hasRole(['admin', 'member']))
                        <form method="POST" action="{{ route('tickets.details', $ticket) }}" enctype="multipart/form-data"
                            class="space-y-4">
                            @csrf
                            <div>
                                ...
                                <select name="error_category"
                                    ...
                                    @foreach(['frontend' => 'Frontend', 'backend' => 'Backend', 'database' => 'Database', 'api' => 'API', 'infrastructure' => 'Infrastructure', 'integration' => 'Integrasi Pihak Ketiga', 'configuration' => 'Konfigurasi', 'other' => 'Lainnya'] as $val => $label)
                                        <option value="{{ $val }}" {{ $ticket->error_category === $val ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <button type="submit"
                                class="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg transition-colors">Simpan</button>
                        </form>
                    @else
                        <div class="space-y-2 text-sm">
                            <div><span class="text-gray-500">Kategori:</span> <span
                                    class="font-medium">{{ $ticket->error_category ? ucfirst($ticket->error_category) : '—' }}</span>
                            </div>
                            <div><span class="text-gray-500">Solusi:</span>
                                <p class="text-gray-600 whitespace-pre-line">{{ $ticket->solution ?: '—' }}</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($ticket->attachments->count())
                        <div class="mt-5 pt-4 border-t border-gray-100">
                            <h5 class="text-xs font-semibold text-gray-500 uppercase mb-2">Lampiran
                                ...
                                @foreach($ticket->attachments as $att)
                                    <div class="flex items-center justify-between gap-2 text-sm bg-gray-50 rounded-lg px-3 py-2">
                                        <a href="{{ $att->url() }}" target="_blank"
                                            class="text-blue-600 hover:underline truncate">{{ $att->file_name }}</a>
                ...
                <div class="hidden bg-white rounded-xl border border-gray-200 p-6">
                    ...
                    @php
                        $linkLabels = [
                            ...
                            'duplicates' => 'Duplikat dari',
                            'duplicated_by' => 'Diduplikasi oleh',
                            'relates_to' => 'Terkait dengan',
                        ...
                        <div class="space-y-2 mb-4">
                            @foreach($ticket->outgoingLinks as $link)
                                <div class="flex items-center justify-between gap-2 text-sm bg-gray-50 rounded-lg px-3 py-2">
                                    <span>
                                        ...
                                        <a href="{{ route('tickets.show', $link->targetTicket) }}"
                                            class="text-blue-600 hover:underline">#{{ $link->targetTicket->id }}
                                            {{ $link->targetTicket->title }}</a>
                                    </span>
                                    @if($user->hasRole(['admin', 'member']))
                                        <form method="POST" action="{{ route('tickets.links.delete', [$ticket, $link]) }}"
                                            ...
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="text-red-500 hover:text-red-700 text-xs flex-shrink-0">Hapus</button>
                                        </form>
                                    @endif
                                </div>
                            @endforeach
                            @foreach($ticket->incomingLinks as $link)
                                <div class="flex items-center justify-between gap-2 text-sm bg-gray-50 rounded-lg px-3 py-2">
                                    <span>
                                        <span class="text-gray-500">{{ $linkLabels[$link->link_type] ?? $link->link_type }}:</span>
                                        <a href="{{ route('tickets.show', $link->sourceTicket) }}"
                                            class="text-blue-600 hover:underline">#{{ $link->sourceTicket->id }}
                                            {{ $link->sourceTicket->title }}</a>
                                    </span>
                                </div>
                            @endforeach
                        </div>
                    ...
                    @if($user->hasRole(['admin', 'member']))
                        <form method="POST" action="{{ route('tickets.links.store', $ticket) }}" class="flex gap-2">
                            ...
                            <select name="target_ticket_id" required
                            ...
                            <button type="submit"
                                class="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg transition-colors flex-shrink-0">Tambah</button>
                        </form>
                    @endif
                </div>
                ...
                @if($ticket->sla_due_at && !in_array($ticket->status, ['resolved', 'closed']))
                    <div class="bg-white rounded-xl border border-gray-200 p-4">
                        ...
                        <div class="flex items-center justify-between text-xs text-gray-500 mb-1">
                            ...
                            <span>Sisa:
                                {{ $slaRemaining > 0 ? floor($slaRemaining / 60) . 'j ' . ($slaRemaining % 60) . 'm' : 'HABIS' }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="{{ $slaColor }} h-2 rounded-full transition-all"
                                style="width: {{ min($slaPercent, 100) }}%"></div>
                        </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    ...
                    <div class="space-y-4 mb-5">
                        @forelse($ticket->comments as $comment)
                            <div class="flex gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold flex-shrink-0">
                    ...
                    <form method="POST" action="{{ route('tickets.comment', $ticket) }}" class="flex gap-3">
                        ...
                        <button type="submit"
                            class="self-end bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg transition-colors">Kirim</button>
                    </form>
                </div>
                        ...
                        <div class="space-y-2">
                            @foreach($ticket->histories->sortByDesc('created_at') as $h)
                                <div class="text-xs text-gray-500 flex gap-2">
                                    ...
                                    <span>mengubah <span
                                            class="font-mono bg-gray-100 px-1 rounded">{{ $h->field_changed }}</span></span>
                                    <span>dari <span class="line-through">{{ $h->old_value ?? '—' }}</span> → <span
                                            class="font-medium text-gray-700">{{ $h->new_value }}</span></span>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3 text-sm">
                    <div><span class="text-gray-500">Proyek:</span> <span
                            class="font-medium">{{ $ticket->project->name }}</span></div>
                    <div><span class="text-gray-500">Tipe:</span> <span
                            class="font-medium">{{ ucfirst($ticket->type) }}</span></div>
                    <div><span class="text-gray-500">Reporter:</span> <span
                            class="font-medium">{{ $ticket->reporter->name }}</span></div>
                    <div><span class="text-gray-500">Assignee:</span> <span
                            class="font-medium">{{ $ticket->assignee->name ?? '—' }}</span></div>
                    <div><span class="text-gray-500">Dibuat:</span>
                        <span>{{ $ticket->created_at->format('d M Y H:i') }}</span></div>
                    @if($ticket->resolved_at)
                        <div><span class="text-gray-500">Resolved:</span>
                            <span>{{ $ticket->resolved_at->format('d M Y H:i') }}</span></div>
                    @endif
                </div>
                            ...
                            <div>
                                ...
                                <select name="assignee_id"
                                    ...
                                    @foreach($developers as $dev)
                                        <option value="{{ $dev->id }}" {{ $ticket->assignee_id === $dev->id ? 'selected' : '' }}>
                                            {{ $dev->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <button type="submit"
                                class="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-3 py-2 rounded-lg transition-colors">Simpan</button>
                        </form>
                    </div>
                @endif
                ...
                @if($ticket->status === 'closed' && $user->hasRole('client'))
                    <div class="bg-white rounded-xl border border-gray-200 p-5" x-data="{open:false}">
                        <button @click="open=!open" class="w-full text-sm font-medium text-red-600 hover:text-red-800">Buka
                            Kembali Tiket</button>
                        <div x-show="open" x-cloak class="mt-3">
                            <form method="POST" action="{{ route('tickets.reopen', $ticket) }}">
                                ...
                                <button type="submit"
                                    class="w-full bg-red-600 hover:bg-red-700 text-white text-sm py-2 rounded-lg transition-colors">Reopen</button>
                            </form>
                        </div>
                    </div>
                @endif
