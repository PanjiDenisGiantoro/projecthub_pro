        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
            <form method="GET" class="flex gap-2 flex-wrap flex-1">
                <select name="status" onchange="this.form.submit()"
                    ...
                    @php
                        $statusLabels = ['waiting_approval' => 'Waiting for Approval', 'approved' => 'Approved', 'rejected' => 'Rejected', 'done' => 'Done'];
                    @endphp
                ...
                <select name="project_id" onchange="this.form.submit()"
                    ...
                    @foreach($projects as $p)
                        <option value="{{ $p->id }}" {{ request('project_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}
                        </option>
                    @endforeach
                </select>
            </form>
                    ...
                    @forelse($requests as $req)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3"><span
                                    class="badge {{ $pc[$req->priority] ?? '' }}">{{ ucfirst($req->priority) }}</span></td>
                            <td class="px-4 py-3"><span
                                    class="badge {{ $sc[$req->status] ?? '' }}">{{ $sl[$req->status] ?? ucfirst(str_replace('_', ' ', $req->status)) }}</span>
                            </td>
