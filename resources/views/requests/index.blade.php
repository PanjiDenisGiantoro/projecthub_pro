    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
        <form method="GET" class="flex gap-2 flex-wrap flex-1">
            <select name="status" onchange="this.form.submit()" class="text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                @php
                    $statusLabels = ['waiting_approval' => 'Waiting for Approval Manager', 'approved' => 'Approved', 'rejected' => 'Rejected', 'done' => 'Done'];
                @endphp
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3"><span class="badge {{ $sc[$req->status] ?? '' }}">{{ $sl[$req->status] ?? ucfirst(str_replace('_',' ',$req->status)) }}</span></td>
