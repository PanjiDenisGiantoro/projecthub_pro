    ...
    <div class="py-4 max-w-4xl">
        <form method="GET" action="{{ route('search.index') }}" class="mb-6">
            <div class="flex gap-3">
                <input type="text" name="q" value="{{ $query }}" autofocus
                    placeholder="Search tasks, projects, tickets, articles..."
                    class="flex-1 px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                <button type="submit"
                    class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl text-sm font-medium transition-colors cursor-pointer">
                    Search
                </button>
            </div>
        </form>
        ...
        @if($results === null)
            <div class="text-center py-12 text-gray-400">
                ...
                <p class="text-gray-500 font-medium">Type keywords to search.</p>
            </div>
        @elseif($totalCount === 0)
            <div class="text-center py-12 text-gray-400">
                <p class="font-medium text-gray-500">No results found for "<strong>{{ $query }}</strong>"</p>
            </div>
        @else
            <p class="text-sm text-gray-500 mb-4">Found <strong>{{ $totalCount }}</strong> results for
            ...
            @if($results['tasks']->count())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <span
                            class="w-5 h-5 bg-blue-100 text-blue-600 rounded text-xs flex items-center justify-center font-bold">T</span>
            ...
            @if($results['projects']->count())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <span
                            class="w-5 h-5 bg-green-100 text-green-600 rounded text-xs flex items-center justify-center font-bold">P</span>
                        ...
                        @foreach($results['projects'] as $proj)
                            <a href="{{ route('projects.show', $proj) }}"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5">Project Lead: {{ $proj->manager?->name }} ·
                                        {{ $proj->status }}</p>
                                </div>
                            </a>
                        @endforeach
            ...
            @if($results['tickets']->count())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <span
                            class="w-5 h-5 bg-red-100 text-red-600 rounded text-xs flex items-center justify-center font-bold">B</span>
            ...
            @if($results['requests']->count())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <span
                            class="w-5 h-5 bg-yellow-100 text-yellow-600 rounded text-xs flex items-center justify-center font-bold">R</span>
            ...
            @if($results['articles']->count())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                        <span
                            class="w-5 h-5 bg-purple-100 text-purple-600 rounded text-xs flex items-center justify-center font-bold">K</span>
