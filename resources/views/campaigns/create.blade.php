        ...
        <form method="POST" action="{{ route('campaigns.store') }}" class="space-y-4"
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="goal_leads" value="{{ old('goal_leads') }}" min="0"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="target" rows="2"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none">{{ old('target') }}</textarea>
            </div>

            <div class="flex gap-3 pt-2">
                <button type="submit" :disabled="submitting" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-6 py-2.5 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed">
