...
<body class="h-full font-sans antialiased" style="background-color:var(--fl-page)" x-data="{
    ...
    <div class="flex h-full">
        ...
        <aside class="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 ph-sidebar transition-all duration-300"
            ...
            @include('layouts.sidebar-subscription')
        </aside>
        ...
        <aside x-show="sidebarOpen" x-cloak class="fixed inset-y-0 left-0 z-50 w-64 flex flex-col lg:hidden ph-sidebar"
            ...
            {{-- Subscription / plan widget --}}
            @include('layouts.sidebar-subscription', ['collapsible' => false])
        </aside>
        ...
        <div class="flex-1 flex flex-col min-h-0 transition-all duration-300"
            ...
            <header class="fl-topbar sticky top-0 z-30 flex items-center h-16 px-5 gap-3 shrink-0">
                ...
                {{-- 7. Profile --}}
                <div x-data="{ userMenuOpen: false }" class="relative group flex items-center">
                    <button @click="userMenuOpen = !userMenuOpen" type="button"
                        class="flex items-center gap-2.5 shrink-0 cursor-pointer">
                        <div class="hidden sm:block text-right leading-tight">
                            <p class="text-[13px] font-semibold truncate max-w-[9rem]" style="color:var(--ph-user-name)">
                                {{ auth()->user()->name }}</p>
                            <p class="text-[11px] truncate max-w-[9rem]" style="color:var(--ph-user-role)">
                                {{ auth()->user()->company->name ?? 'Workspace' }}</p>
                        </div>
                        @if(auth()->user()->avatar)
                            <img src="{{ Storage::url(auth()->user()->avatar) }}" alt="{{ auth()->user()->name }}"
                                class="w-9 h-9 rounded-full object-cover ring-2 ring-white/20 group-hover:ring-purple-400 transition-all shrink-0">
                        @else
                            <div
                                class="fl-avatar w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-xs ring-2 ring-white/20 group-hover:ring-purple-400 transition-all shrink-0">
                                {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                            </div>
                        @endif
                    </button>
                    <span x-show="!userMenuOpen"
