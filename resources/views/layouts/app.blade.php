...
<body class="h-full font-sans antialiased" style="background-color:var(--fl-page)" x-data="{
    ...
    <div id="page-loader" class="fixed inset-0 z-[9999] overflow-hidden" style="background-color:#ffffff">
        <div id="page-loader-icon" class="absolute inset-0 flex items-center justify-center">
            <img src="{{ asset('flovig_loading_white.gif') }}" alt="Loading..."
                class="w-72 sm:w-80 h-auto object-contain">
        </div>
    </div>
    ...
    <div class="flex h-full">
        ...
        <aside class="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 ph-sidebar transition-all duration-300"
            :class="sidebarCollapsed ? 'sidebar-collapsed lg:w-20' : 'lg:w-64'" style="z-index:20">
            ...
            <div class="flex items-center h-16 shrink-0 ph-side-divider-b transition-all duration-300"
                ...
                {{-- Expanded: Full logo --}}
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 overflow-hidden" title="Flovig"
                    x-show="!sidebarCollapsed">
                    <img src="{{ asset('flovig_logo.png') }}" alt="Flovig" class="h-7 w-auto object-contain shrink-0">
                </a>
                {{-- Collapsed: Icon logo --}}
                <a href="{{ route('dashboard') }}" class="flex items-center justify-center" title="Flovig"
                    x-show="sidebarCollapsed" x-cloak>
                    <img src="{{ asset('flovig_icon.png') }}" alt="Flovig"
                ...
                <button type="button" @click="sidebarCollapsed = !sidebarCollapsed"
                    ...
                    title="Collapse Sidebar" x-show="!sidebarCollapsed">
            ...
            <div class="px-3 py-3 shrink-0 ph-side-divider-t">
                <div x-data="{ userMenuOpen: false }" class="relative">
                    <button @click="userMenuOpen = !userMenuOpen"
                        ...
                        @else
                            <div
                                class="fl-avatar w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0">
                                {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                            </div>
                        @endif
                        <div class="flex-1 min-w-0">
                            <p class="text-[13px] font-semibold truncate leading-tight"
                                style="color:var(--ph-user-name)">{{ auth()->user()->name }}</p>
                            <p class="text-[11px] truncate capitalize leading-tight mt-0.5"
                                style="color:var(--ph-user-role)">
                                {{ \App\Support\RoleLabel::for(auth()->user()->getRoleNames()->first()) }}</p>
                        </div>
                        <svg class="w-3.5 h-3.5 shrink-0 transition-transform duration-200"
                            style="color:var(--ph-user-chev)" :class="userMenuOpen ? 'rotate-180' : ''" fill="none"
                    ...
                    <div x-show="userMenuOpen" x-cloak @click.away="userMenuOpen = false"
                        ...
                        <div class="px-4 py-3 ph-drop-divider-b">
                            <p class="text-[13px] font-semibold truncate" style="color:var(--ph-user-name)">
                                {{ auth()->user()->name }}</p>
                            <p class="text-[11px] truncate" style="color:var(--ph-drop-email)">
                                {{ auth()->user()->email }}</p>
                        </div>
                        <div class="p-1.5">
        ...
        <aside x-show="sidebarOpen" x-cloak class="fixed inset-y-0 left-0 z-50 w-64 flex flex-col lg:hidden ph-sidebar"
            ...
            <div class="px-3 py-3 shrink-0 ph-side-divider-t">
                <div class="flex items-center gap-3 px-3 py-2">
                    ...
                    @endif
                    <div class="flex-1 min-w-0">
                        <p class="text-[13px] font-semibold truncate leading-tight" style="color:var(--ph-user-name)">
                            {{ auth()->user()->name }}</p>
                        <p class="text-[11px] truncate capitalize leading-tight mt-0.5"
                            style="color:var(--ph-user-role)">
                            {{ \App\Support\RoleLabel::for(auth()->user()->getRoleNames()->first()) }}</p>
                    ...
                    @if(auth()->user()->is_super_admin)
                        <a href="{{ route('superadmin.dashboard') }}"
                            class="text-slate-400 hover:text-blue-400 p-1.5 rounded-lg hover:bg-blue-500/10 transition-colors shrink-0"
                            title="Superadmin">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="text-slate-400 hover:text-red-400 p-1.5 rounded-lg hover:bg-red-500/10 transition-colors shrink-0 cursor-pointer"
                            title="Sign out">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
        ...
        <div class="flex-1 flex flex-col min-h-0 transition-all duration-300"
            ...
            <header class="fl-topbar sticky top-0 z-30 flex items-center h-16 px-5 gap-3 shrink-0">
                <button type="button" @click="sidebarOpen = true"
                    class="fl-hamburger lg:hidden p-1.5 rounded-lg transition-colors cursor-pointer" title="Open Menu">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 6h16M4 12h16M4 18h16" />
                    </svg>
            ...
            @if(auth()->check() && !auth()->user()->is_super_admin && !auth()->user()->hasRole('admin') && !auth()->user()->isLifetime() && !auth()->user()->isExpired())
                ...
                @if($daysLeft <= 14)
                    <div x-data="{ show: !localStorage.getItem('ph_banner_dismissed_{{ now()->toDateString() }}') }"
                        ...
                        <svg class="w-4 h-4 shrink-0 {{ $daysLeft <= 3 ? 'text-red-500' : 'text-amber-500' }}" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
                        </svg>
                        @if($daysLeft <= 0)
                            <span>Your account active period <strong>expires today</strong>. Please contact your administrator to
                                renew.</span>
                        @elseif($daysLeft === 1)
                            <span>Your account active period <strong>expires tomorrow</strong>. Please contact your
                                administrator.</span>
                        @else
                        ...
                        @endif
                        <button
                            @click="show = false; localStorage.setItem('ph_banner_dismissed_{{ now()->toDateString() }}', '1')"
            ...
            <main class="@yield('main-class', 'flex-1 px-6 pb-8 overflow-auto')">
                @if(auth()->check() && !auth()->user()->is_super_admin && auth()->user()->isCompanyExpiringSoon())
                    ...
                    <div
                        class="mt-4 mb-2 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-sm flex items-center gap-2">
    ...
    <div x-data="aiAssistantWidget()" x-cloak>
        ...
        <div x-show="open" x-cloak x-transition
            ...
            <div class="flex items-center gap-3 px-4 py-3 shrink-0" style="background:var(--ai-gradient)">
                ...
                <button @click="minimized = !minimized" :title="minimized ? 'Maximize' : 'Minimize'"
                    class="p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition cursor-pointer">
                    <svg x-show="!minimized" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14" />
                    </svg>
                    <svg x-show="minimized" x-cloak class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                    </svg>
                </button>
                <button x-show="!minimized" @click="expanded = !expanded" :title="expanded ? 'Minimize' : 'Expand size'"
                    class="p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition cursor-pointer">
                    <svg x-show="!expanded" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
                    </svg>
                    <svg x-show="expanded" x-cloak class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 9V5m0 4H5m4 0L4 4m11 5V5m0 4h4m-4 0l5-5M9 15v4m0-4H5m4 0l-5 5m11-5v4m0-4h4m-4 0l5 5" />
                    </svg>
                </button>
                <button x-show="!minimized" @click="clearHistory()" title="Clear history"
                    class="p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition cursor-pointer">
            ...
            {{-- Messages --}}
            <div x-show="!minimized" class="flex-1 overflow-y-auto px-4 py-4 space-y-3" x-ref="aiMsgArea"
                style="background:var(--fl-search-bg,#f9fafb)">
                    ...
                    <div>
                        ...
                        <template x-if="m.action">
                            <div class="rounded-2xl border p-3 space-y-2"
                                ...
                                <p class="text-xs font-bold uppercase tracking-wide" style="color:#7c3aed">Suggested
                                    Action</p>
                                <p class="text-sm font-semibold" style="color:var(--fl-text-h,#1a0a3d)"
                                    x-text="m.action.label"></p>
                                <div class="text-xs space-y-0.5" style="color:var(--fl-text-muted,#6b7280)">
                                    ...
                                    <p x-show="m.action.args.description"><strong>Description:</strong> <span
                                            x-text="m.action.args.description"></span></p>
                                </div>
                                ...
                                <p x-show="m.action.status === 'done'" class="text-xs font-medium" style="color:#10b981"
                                    x-text="m.action.resultMessage"></p>
                                <a x-show="m.action.status === 'done' && m.action.url" :href="m.action.url"
                                    class="text-xs font-semibold underline" style="color:#7c3aed">Open project →</a>
                                <p x-show="m.action.status === 'error'" class="text-xs font-medium"
                                    style="color:#ef4444" x-text="m.action.resultMessage"></p>
                                <p x-show="m.action.status === 'cancelled'" class="text-xs italic"
                                    style="color:var(--fl-text-muted,#6b7280)">Cancelled.</p>
                                <span x-show="m.ts" class="block text-[10px]" style="color:var(--fl-text-muted,#9ca3af)"
                                    x-text="formatTs(m.ts)"></span>
                            </div>
                        </template>

                        {{-- Plain text bubble --}}
                        <div x-show="!m.action" class="flex flex-col"
                            ...
                            <span x-show="m.ts" class="text-[10px] mt-0.5 px-1"
                                style="color:var(--fl-text-muted,#9ca3af)" x-text="formatTs(m.ts)"></span>
                        </div>
                    </div>
                </template>
                ...
                <div x-show="thinking" class="flex justify-start">
                    <div class="px-3 py-2 rounded-2xl rounded-bl-sm border flex items-center gap-1"
                        ...
                        <span class="w-1.5 h-1.5 rounded-full animate-bounce"
                            style="background:#a78bfa;animation-delay:0ms"></span>
                        <span class="w-1.5 h-1.5 rounded-full animate-bounce"
                            style="background:#a78bfa;animation-delay:150ms"></span>
                        <span class="w-1.5 h-1.5 rounded-full animate-bounce"
                            style="background:#a78bfa;animation-delay:300ms"></span>
                    </div>
                </div>
            </div>

            {{-- Input --}}
            <div x-show="!minimized" class="p-3 border-t shrink-0"
                style="border-color:var(--fl-card-border,#ede9fe);background:var(--fl-card-bg,#fff)">
                <div class="flex gap-2 items-end">
                    <textarea x-model="input" x-ref="aiInput"
                        @keydown.enter="if(!$event.shiftKey){$event.preventDefault();send();}" rows="1"
    ...
    <div id="fl-sidebar-tooltip"
        class="pointer-events-none fixed z-[99999] opacity-0 transition-opacity duration-150 whitespace-nowrap px-2.5 py-1 text-[11px] font-medium rounded-md text-white bg-slate-900/95 shadow-xl border border-white/10 hidden">
    </div>
    <script>
        (function () {
