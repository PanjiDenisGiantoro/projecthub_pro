...
<div class="py-4 max-w-xl">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center gap-2 mb-5">
            <span class="text-xs px-2 py-1 bg-blue-50 text-blue-600 rounded font-mono">L{{ $organizationUnit->code }}</span>
        ...
        <form method="POST" action="{{ route('organization-units.update', $organizationUnit) }}" class="space-y-5"
            ...
            <div>
                ...
                <select name="parent_id" id="sel-parent"
                    ...
                    @foreach($tree as $node)
                        <option value="{{ $node->id }}" data-level="{{ $node->level }}" {{ (string) $organizationUnit->parent_id === (string) $node->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" id="is_active"
                       {{ old('is_active', $organizationUnit->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded">
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <label class="flex items-center gap-2 text-sm text-gray-600">
                        <input type="checkbox" id="color-auto" onchange="toggleColorAuto(this)" {{ old('color', $organizationUnit->color) ? '' : 'checked' }} class="w-4 h-4 text-blue-600 rounded">
                    ...
                    <input type="color" name="color" id="color-input"
                           ...
                           class="w-12 h-9 border border-gray-300 rounded-lg cursor-pointer disabled:cursor-not-allowed disabled:opacity-50" {{ old('color', $organizationUnit->color) ? '' : 'disabled' }}>
                </div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-6 py-2.5 rounded-lg transition-colors">Simpan Perubahan</button>
