    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form method="POST" action="{{ route('divisions.store') }}" class="space-y-5"
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium px-6 py-2.5 rounded-lg transition-colors">Simpan Divisi</button>
...
$(function () {
    ...
    $('#sel-company-filter').select2({ placeholder: '— Filter by Perusahaan (opsional) —', allowClear: true, width: '100%' });
    ...
    const oldBranchId = '{{ old('branch_id') }}' || null;
    renderBranches(null, oldBranchId);
});
</script>
@endpush
