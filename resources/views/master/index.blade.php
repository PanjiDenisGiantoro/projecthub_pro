...
<div class="py-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
        ...
        <a href="{{ route('organization-units.index') }}"
           class="group bg-white rounded-xl border border-gray-200 p-4 hover:border-indigo-300 hover:shadow-sm transition-all">
            ...
            <p class="text-xs text-gray-500 font-medium">Unit Organisasi</p>
        </a>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 overflow-x-auto">
            <div class="space-y-6" style="min-width: 480px;">
                ...
                @php
                    ...
                    $colorMap = [
                        ...
                        'cyan'    => ['light'=>'bg-cyan-50',    'text'=>'text-cyan-700'],
                ...
                <div>
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 rounded-xl {{ $c['light'] }} {{ $c['text'] }} flex items-center justify-center font-bold text-sm flex-shrink-0">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-400">{{ $company->orgTree->count() }} unit organisasi</p>
                        </div>
                    ...
                    {{-- Units: flat list terurut DFS, indentasi mengikuti level --}}
                    @if($company->orgTree->isNotEmpty())
                            ...
                            @if($unit->head)
                                <div class="flex items-center gap-1 text-xs text-gray-400">
                                    <div class="w-4 h-4 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold">
                                        {{ strtoupper(substr($unit->head->name, 0, 1)) }}
                                    </div>
                                    <span>{{ explode(' ', $unit->head->name)[0] }}</span>
                                </div>
                            @endif
                            <span class="text-xs text-gray-400">{{ $unit->users_count }} user</span>
                            <a href="{{ route('organization-units.edit', $unit) }}" class="text-xs text-gray-400 hover:text-blue-600 transition-colors opacity-0 group-hover:opacity-100">Edit</a>
                        </div>
                        @endforeach
