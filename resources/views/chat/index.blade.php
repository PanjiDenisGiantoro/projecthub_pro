...
@section('content')
@php
    // Tab "Proyek" cuma relevan di konteks paket Task Management. Dicek dari
    // active_package (bukan cuma kepemilikan paket) — supaya company yang punya
    // kedua paket dan sedang toggle ke tampilan HRIS juga tidak lihat tab ini,
    // sama seperti pola @if(session('active_package') === 'hris') di halaman HRIS lain.
    $showProjectTab = session('active_package') !== 'hris';
@endphp
<div class="flex h-full overflow-hidden bg-gray-50"
    ...
    <div class="w-72 shrink-0 flex flex-col border-r border-gray-200 bg-white"
        ...
        <div class="px-4 pt-5 pb-3 border-b border-gray-100 shrink-0">
            <div class="flex items-center justify-between mb-4">
                <h1 class="text-lg font-bold text-gray-900 tracking-tight">Chat</h1>
                <button x-show="tab === 'forum'" x-cloak @click="openCreateForum()"
                        class="w-7 h-7 flex items-center justify-center rounded-lg text-white shadow-sm hover:opacity-90 active:scale-95 transition-all"
                        style="background:linear-gradient(135deg,#2563eb,#0891b2)" title="Buat forum baru">
            ...
            <div class="flex gap-1 mb-3 bg-gray-100 rounded-xl p-1">
                ...
                <button @click="tab = 'project'"
                        ...
                        class="flex-1 text-xs font-semibold py-1.5 rounded-lg transition-all relative">
                    Proyek
            ...
            <div class="relative">
                <input type="text"
                       ...
                       class="w-full pl-8 pr-3 py-2 text-sm border border-transparent rounded-xl bg-gray-100 focus:bg-white focus:border-blue-200 focus:ring-2 focus:ring-blue-100 outline-none transition-all">
        ...
        <div class="flex-1 overflow-y-auto py-1">
            ...
            <div x-show="filteredList.length === 0" class="px-4 py-10 text-center">
            ...
            <template x-for="item in filteredList" :key="item.type + '-' + item.id">
                <button @click="selectItem(item)"
                        :class="isActive(item)
                            ? 'bg-blue-50/70 border-r-[3px] border-blue-500'
                    ...
                    <div class="relative shrink-0">
                        <div class="w-11 h-11 rounded-2xl flex items-center justify-center font-bold text-white text-sm overflow-hidden transition-shadow"
                             :class="isActive(item) ? 'ring-2 ring-blue-300 ring-offset-2 ring-offset-white' : ''"
                             :style="item.type === 'forum'
                                 ? 'background:linear-gradient(135deg,#f59e0b,#d97706)'
                                 : (item.type === 'project' ? 'background:linear-gradient(135deg,#0ea5e9,#2563eb)' : 'background:linear-gradient(135deg,#64748b,#475569)')">
                        ...
                        <template x-if="item.unread_count > 0">
                            <span class="absolute -top-1 -right-1 bg-blue-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-sm"
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-1 mb-0.5">
                            ...
                            <span x-show="item.last_message" class="shrink-0 text-[10px] text-gray-300" x-text="item.last_message?.time"></span>
                        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden"
         style="background:radial-gradient(circle at top left, rgba(99,102,241,0.04), transparent 45%), #f8fafc"
        ...
        <div x-show="activeItem" class="flex-1 flex flex-col overflow-hidden">
            ...
            {{-- Thread header --}}
            <div class="px-5 py-3 bg-white/90 backdrop-blur border-b border-gray-200 flex items-center gap-3 shrink-0">
                ...
                <div class="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-white text-sm shrink-0 overflow-hidden"
                     :style="activeItem?.type === 'forum'
                         ? 'background:linear-gradient(135deg,#f59e0b,#d97706)'
                         : (activeItem?.type === 'project' ? 'background:linear-gradient(135deg,#0ea5e9,#2563eb)' : 'background:linear-gradient(135deg,#64748b,#475569)')">
                ...
                <span class="text-xs text-gray-400 font-medium" x-text="messages.filter(m => !m.deleted).length + ' pesan'"></span>
            </div>
            ...
            <div x-show="!loading"
                 x-ref="msgArea"
                 class="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-0.5">
                    ...
                    <div>
                        <template x-if="idx === 0 || messages[idx-1].date_label !== msg.date_label">
                            <div class="flex items-center gap-3 py-3">
                                ...
                                <span class="text-[11px] text-gray-400 font-semibold px-2.5 py-1 bg-white rounded-full border border-gray-200 shadow-sm" x-text="msg.date_label"></span>
                                <div class="flex-1 h-px bg-gray-200"></div>
                            </div>
                        </template>
                        ...
                        <template x-if="msg.deleted">
                            <div class="flex justify-center py-1">
                                <span class="inline-flex items-center gap-1.5 text-xs text-gray-400 italic bg-gray-100 rounded-full px-3 py-1 select-none">
                        ...
                        <template x-if="!msg.deleted">
                            <div class="group w-full flex gap-2 py-1" :class="msg.is_mine ? 'flex-row-reverse' : 'flex-row'">

                                <div class="w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-white text-xs font-bold self-end mb-5 overflow-hidden"
                                     x-show="!msg.is_mine"
                                ...
                                <div class="flex flex-col max-w-[75%] sm:max-w-[65%]" :class="msg.is_mine ? 'items-end' : 'items-start'">
                                    <div class="flex items-baseline gap-2 mb-1 px-1" :class="msg.is_mine ? 'flex-row-reverse' : ''">
                                        <span x-show="!msg.is_mine" class="text-xs font-semibold text-gray-600" x-text="msg.user.name"></span>
                                        <span class="text-[11px] text-gray-400" x-text="msg.time_label"></span>
                                        <span x-show="msg.edited_at" class="text-[10px] text-gray-300 italic">diedit</span>
                                    </div>

                                    <div class="relative rounded-2xl px-3.5 py-2 shadow-sm"
                                         :class="msg.is_mine
                                             ? 'text-white rounded-br-md'
                                             : 'bg-white border border-gray-200 text-gray-800 rounded-bl-md'"
                                         :style="msg.is_mine ? 'background:linear-gradient(135deg,#2563eb,#0891b2)' : ''">

                                        <template x-if="msg.parent">
                                            <div class="mb-1.5 rounded-lg px-2.5 py-1.5 border-l-2"
                                                 :class="msg.is_mine ? 'border-white/50 bg-white/10' : 'border-blue-300 bg-blue-50'">
                                                <span class="text-[11px] font-semibold" :class="msg.is_mine ? 'text-white/85' : 'text-blue-600'" x-text="msg.parent.user"></span>
                                                <p class="text-xs truncate max-w-[220px]" :class="msg.is_mine ? 'text-white/70' : 'text-gray-500'" x-text="msg.parent.body"></p>
                                            </div>
                                        </template>

                                        <template x-if="editingId !== msg.id">
                                            <p class="text-sm leading-relaxed whitespace-pre-wrap break-words" :class="msg.is_mine ? 'text-white' : 'text-gray-800'" x-text="msg.body"></p>
                                        </template>
                                        <template x-if="editingId === msg.id">
                                            <div class="min-w-[220px]">
                                                <textarea x-model="editBody"
                                                          ...
                                                          class="w-full border border-blue-300 rounded-lg px-2.5 py-2 text-sm text-gray-800 focus:ring-2 focus:ring-blue-200 outline-none resize-none"
                                                          rows="2"></textarea>
                                                <div class="flex gap-2 mt-1.5">
                                                    ...
                                                    <button @click="cancelEdit()" class="text-xs px-3 py-1 rounded-lg transition" :class="msg.is_mine ? 'text-white/80 hover:text-white' : 'text-gray-500 hover:text-gray-700'">Batal</button>
                                                </div>
                                            </div>
                                        </template>
                                    </div>
                                ...
                                <div class="invisible group-hover:visible shrink-0 flex items-center gap-0.5 self-center">
                                    <button @click="setReply(msg)" class="p-1.5 rounded-lg text-gray-300 hover:text-blue-500 hover:bg-white hover:shadow-sm transition" title="Balas">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
                                    </button>
                                    <button x-show="msg.is_mine && editingId !== msg.id" @click="startEdit(msg)" class="p-1.5 rounded-lg text-gray-300 hover:text-blue-500 hover:bg-white hover:shadow-sm transition" title="Edit">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                    </button>
                                    <button x-show="msg.is_mine || (activeItem?.type !== 'dm')" @click="deleteMessage(msg)" class="p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-white hover:shadow-sm transition" title="Hapus">
            ...
            {{-- Reply indicator --}}
            <div x-show="replyTo" x-cloak class="px-5 py-2.5 bg-blue-50 border-t border-blue-100 flex items-center gap-3 shrink-0">
            ...
            {{-- Input area --}}
            <div class="px-5 py-3.5 border-t border-gray-200 shrink-0 bg-white">
                <div class="flex gap-2.5 items-end">
                    <div class="flex-1 relative">
                        <textarea x-model="newBody"
                                  ...
                                  class="w-full border border-gray-200 rounded-2xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-300 outline-none resize-none leading-relaxed transition-all bg-gray-50 focus:bg-white"
                                  style="max-height:120px; overflow-y:auto"></textarea>
                    </div>

                    <button @click="send()"
                            :disabled="sending || !newBody.trim()"
                            class="shrink-0 w-10 h-10 rounded-full text-white flex items-center justify-center shadow-sm hover:shadow-md active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed disabled:shadow-none transition-all"
                            style="background:linear-gradient(135deg,#2563eb,#0891b2)" title="Kirim">
                        <svg x-show="!sending" class="w-4 h-4 translate-x-[-1px]" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M2.94 2.94a1.5 1.5 0 011.61-.33l17 7a1.5 1.5 0 010 2.78l-17 7a1.5 1.5 0 01-2.03-1.81L3.87 12 2.52 4.75a1.5 1.5 0 01.42-1.81z"/>
                        </svg>
    ...
    <div x-show="createForumOpen" x-cloak
        ...
        <div class="relative w-full max-w-md rounded-2xl overflow-hidden shadow-2xl bg-white" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button @click="createForumOpen = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition">
            ...
            <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                    ...
                    <input type="text" x-model="newForumName" placeholder="cth: Diskusi Umum"
                           class="w-full px-3 py-2.5 text-sm border border-transparent rounded-xl bg-gray-100 focus:bg-white focus:border-blue-200 focus:ring-2 focus:ring-blue-100 outline-none transition-all">
                </div>
                <div>
                    ...
                    <textarea x-model="newForumDesc" rows="2" placeholder="Tentang apa forum ini…"
                              class="w-full px-3 py-2.5 text-sm border border-transparent rounded-xl bg-gray-100 focus:bg-white focus:border-blue-200 focus:ring-2 focus:ring-blue-100 outline-none transition-all resize-none"></textarea>
                </div>
                <div>
            ...
            <div class="px-6 py-4 border-t border-gray-100 flex gap-2 justify-end">
                ...
                <button @click="createForum()" :disabled="!newForumName.trim() || creatingForum"
                        class="px-5 py-2 text-white text-sm font-semibold rounded-xl hover:opacity-90 disabled:opacity-40 transition-all shadow-sm"
    ...
    <div x-show="membersOpen" x-cloak
        ...
        <div class="relative w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl bg-white" @click.stop>
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                ...
                <button @click="membersOpen = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <div class="p-4 border-t border-gray-100">
                ...
                <select x-model="addMemberId" @change="addForumMember()"
                        class="w-full px-3 py-2 text-sm border border-transparent rounded-xl bg-gray-100 focus:bg-white focus:border-blue-200 outline-none transition-all">
...
document.addEventListener('alpine:init', () => {
    Alpine.data('chatHubApp', (projectsData, dmsData, forumsData, allPeers, showProjectTab) => ({
        tab: showProjectTab ? 'project' : 'dm',
        search: '',
