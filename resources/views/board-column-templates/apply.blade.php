@extends('layouts.app')
@section('title', 'Terapkan Template Kolom')
...
<div class="py-4 max-w-xl">
    <nav class="text-sm text-gray-500 mb-4">
        ...
        <a href="{{ route('board-column-templates.show', $template) }}" class="hover:text-blue-600">{{ $template->name }}</a>
        <span class="mx-2">/</span>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <p class="text-sm text-gray-500 mb-5">Template ini akan membuat {{ $template->items->count() }} kolom board pada proyek yang dipilih. Proyek yang sudah punya kolom board tidak bisa ditimpa — hapus kolom yang ada dulu jika ingin mengganti.</p>

        <form method="POST" action="{{ route('board-column-templates.apply.post', $template) }}" class="space-y-4">
            @csrf
            <div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('board-column-templates.show', $template) }}" class="text-sm text-gray-600 hover:text-gray-800 px-6 py-2.5 rounded-lg border border-gray-300">Batal</a>
            </div>
        </form>
    </div>
</div>
