...
<div class="max-w-2xl mx-auto pt-5 space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">{{ $user->name }} — berlaku sejak {{ $salary->effective_date->format('d M Y') }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <form action="{{ route('hris.salary.update', [$user, $salary]) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')
            ...
            @unless(\App\Support\EmploymentType::defaultBpjsEnrollment($user->employment_type))
            <div class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800">
                ...
                — BPJS biasanya sudah ditanggung perusahaan asal/vendor, bukan perusahaan ini.
            </div>
            @endunless
