...
@section('content')
<div class="py-4 w-full" x-data="templateBuilder()">
    ...
    <form method="POST" action="{{ route('templates.store') }}" class="space-y-5">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="category" placeholder="e.g. Website, Mobile App..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="sm:col-span-2">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" @click="addMilestone()" class="text-sm text-blue-600 hover:text-blue-800 font-medium">+ Milestone</button>
            </div>
                ...
                <div class="border border-gray-200 rounded-xl p-4 mb-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" :name="'milestones['+mi+'][title]'" x-model="ms.title" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Mulai (hari ke)</label>
                            <input type="number" :name="'milestones['+mi+'][offset_days]'" x-model="ms.offset_days" min="0" value="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Durasi (hari)</label>
                            <input type="number" :name="'milestones['+mi+'][duration_days]'" x-model="ms.duration_days" min="1" value="7" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    ...
                    <div class="pl-4 border-l-2 border-gray-100">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <button type="button" @click="ms.tasks.push({title:'',priority:'medium',estimated_hours:'',story_points:''})" class="text-xs text-blue-600 hover:text-blue-800">+ Task</button>
                        </div>
                        <template x-for="(task, ti) in ms.tasks" :key="ti">
                            <div class="flex gap-2 mb-2 items-center">
                                <input type="text" :name="'milestones['+mi+'][tasks]['+ti+'][title]'" x-model="task.title" placeholder="Judul task" required class="flex-1 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                                <select :name="'milestones['+mi+'][tasks]['+ti+'][priority]'" x-model="task.priority" class="px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                                    <option value="low">Low</option>
                                    <option value="medium" selected>Medium</option>
                                    <option value="high">High</option>
                                    <option value="critical">Critical</option>
                                </select>
                                <input type="number" :name="'milestones['+mi+'][tasks]['+ti+'][estimated_hours]'" x-model="task.estimated_hours" placeholder="Jam" min="0" class="w-16 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                                <input type="number" :name="'milestones['+mi+'][tasks]['+ti+'][story_points]'" x-model="task.story_points" placeholder="Pts" min="0" class="w-14 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
