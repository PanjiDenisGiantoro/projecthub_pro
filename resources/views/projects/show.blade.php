...
.select2-dropdown { border: 1px solid #d1d5db !important; border-radius: 0.75rem !important; font-size: 0.875rem !important; }
.select2-results__option--highlighted { background-color: #3b82f6 !important; }
.select2-search--dropdown .select2-search__field { border-radius: 0.5rem !important; border: 1px solid #d1d5db !important; padding: 0.375rem 0.625rem !important; font-size: 0.875rem !important; }
...
@section('content')
<div class="py-6 w-full max-w-full min-w-0 overflow-x-hidden"
     x-data="projectPageData()"
    ...
    <div class="flex flex-col lg:flex-row gap-6 items-start w-full max-w-full min-w-0">
        ...
        ============================================================ --}}
        <aside class="w-full lg:w-56 shrink-0 lg:sticky lg:top-4 flex flex-col justify-between min-h-[calc(100vh-120px)]">
            <div class="space-y-4">
                {{-- Top Project Header/Breadcrumb --}}
                <div class="px-2 pb-2 border-b border-gray-100 dark:border-gray-800">
                    <p class="text-[11px] font-semibold text-gray-400">Project &gt; <span class="text-gray-700 dark:text-gray-300 font-bold truncate">{{ $project->name }}</span></p>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <nav class="space-y-0.5">
                        ...
                        <button
                            ...
                            <span x-show="tab === '{{ $t['key'] }}'" class="w-1.5 h-1.5 rounded-full bg-blue-600 dark:bg-blue-400 shrink-0"></span>
                        </button>
                        @endforeach
                    </nav>
                </div>
        ...
        <div class="flex-1 min-w-0 w-full max-w-full space-y-5 overflow-x-hidden">
            ...
            <div x-show="tab !== 'tasks'" x-cloak class="space-y-5">
                {{-- Top Header Card --}}
                <div class="bg-white dark:bg-gray-850 rounded-2xl shadow-2xs border border-gray-200/90 dark:border-gray-700/80 p-5 sm:p-6 space-y-4">
                    {{-- Breadcrumb Line --}}
                    <div class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
                        ...
                        <span>&gt;</span>
                        <span class="text-blue-600 dark:text-blue-400 font-semibold" x-text="tab.charAt(0).toUpperCase() + tab.slice(1)">Overview</span>
                    </div>

                    {{-- Title Row + Action Buttons --}}
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex items-center gap-3 flex-wrap">
                            <h1 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight">{{ $project->name }}</h1>
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border {{ $sc['class'] }}">
                    ...
                    {{-- Meta Info Line --}}
                    <div class="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-gray-500 dark:text-gray-400 pt-1">
                        <span class="flex items-center gap-1.5">
                            ...
                            Client: <strong class="text-gray-800 dark:text-gray-200 font-semibold">{{ $project->client?->name ?? 'Client One Global' }}</strong>
                        </span>
                        <span class="flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            Lead: <strong class="text-gray-800 dark:text-gray-200 font-semibold">{{ $project->manager?->name ?? 'Admin ProjectHub' }}</strong>
                        </span>
                        <span class="flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                            Timeline: <strong class="text-gray-800 dark:text-gray-200 font-semibold">
                                {{ $project->start_date ? \Carbon\Carbon::parse($project->start_date)->format('d M Y') : '01 Sep 2026' }} – {{ $project->end_date ? \Carbon\Carbon::parse($project->end_date)->format('d M Y') : '30 Nov 2026' }}
                            </strong>
                        </span>
                    </div>
                    ...
                    <div class="pt-2 space-y-1.5">
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="font-extrabold text-blue-600 dark:text-blue-400">{{ $progress }}% Complete</span>
                        </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {{-- Card 1: TOTAL TASKS --}}
                    <div class="bg-white dark:bg-gray-850 rounded-2xl border border-gray-200/90 dark:border-gray-700/80 p-4 shadow-2xs flex items-center justify-between gap-3">
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">TOTAL TASKS</p>
                            <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">{{ $totalTasks }}</p>
                        </div>
                        <div class="w-11 h-11 rounded-xl bg-blue-50 dark:bg-blue-950/50 border border-blue-100 dark:border-blue-900/60 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0">
                    ...
                    <div class="bg-white dark:bg-gray-850 rounded-2xl border border-gray-200/90 dark:border-gray-700/80 p-4 shadow-2xs flex items-center justify-between gap-3">
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">COMPLETED</p>
                            <p class="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1">{{ $doneTasks }}</p>
                        </div>
                        <div class="w-11 h-11 rounded-xl bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-100 dark:border-emerald-900/60 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shrink-0">
                    ...
                    <div class="bg-white dark:bg-gray-850 rounded-2xl border border-gray-200/90 dark:border-gray-700/80 p-4 shadow-2xs flex items-center justify-between gap-3">
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">SPRINT MEMBERS</p>
                            <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">{{ $project->members->count() ?: 8 }}</p>
                        </div>
                        <div class="w-11 h-11 rounded-xl bg-purple-50 dark:bg-purple-950/50 border border-purple-100 dark:border-purple-900/60 flex items-center justify-center text-purple-600 dark:text-purple-400 shrink-0">
                    ...
                    <div class="bg-white dark:bg-gray-850 rounded-2xl border border-gray-200/90 dark:border-gray-700/80 p-4 shadow-2xs flex items-center justify-between gap-3">
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">WORKING DAYS LEFT</p>
                            <p class="text-2xl font-black text-blue-600 dark:text-blue-400 mt-1">
                                {{ $daysLeft !== null ? $daysLeft.'d' : '4d' }}
                            </p>
                        </div>
                        <div class="w-11 h-11 rounded-xl bg-amber-50 dark:bg-amber-950/50 border border-amber-100 dark:border-amber-900/60 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0">
            ...
            <div x-show="tab === 'tasks'" x-cloak class="space-y-4 w-full max-w-full min-w-0">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                    ...
                    <div class="inline-flex items-center gap-1 bg-gray-100/90 dark:bg-gray-800 p-1 rounded-xl shrink-0">
                        <button type="button"
                            ...
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2H4V5zm0 4h16v10a1 1 0 01-1 1H5a1 1 0 01-1-1V9z"/></svg>
                            Board
                        </button>
                        <button type="button"
                                @click="taskView = 'list'"
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false" type="button"
                                ...
                                <span>Assignee: <strong class="font-semibold" x-text="filterAssigneeName || 'All'"></strong></span>
                                <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            <div x-show="open" x-cloak class="absolute right-0 top-full mt-1.5 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-1.5 z-40 space-y-0.5">
                                <button @click="filterAssignee = ''; filterAssigneeName = 'All'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700" :class="filterAssignee === '' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">All Assignees</button>
                                @foreach($assignableUsers as $u)
                                <button @click="filterAssignee = '{{ $u->id }}'; filterAssigneeName = '{{ addslashes($u->name) }}'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2" :class="filterAssignee === '{{ $u->id }}' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">
                                    <span class="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center text-[9px] font-bold">{{ strtoupper(substr($u->name, 0, 1)) }}</span>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false" type="button"
                                ...
                                <span>Due Date: <strong class="font-semibold" x-text="filterDueDateName || 'Any time'"></strong></span>
                                <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            <div x-show="open" x-cloak class="absolute right-0 top-full mt-1.5 w-44 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-1.5 z-40 space-y-0.5">
                                <button @click="filterDueDate = ''; filterDueDateName = 'Any time'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700" :class="filterDueDate === '' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">Any time</button>
                                <button @click="filterDueDate = 'today'; filterDueDateName = 'Today'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700" :class="filterDueDate === 'today' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">Today</button>
                                <button @click="filterDueDate = 'week'; filterDueDateName = 'This Week'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700" :class="filterDueDate === 'week' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">This Week</button>
                                <button @click="filterDueDate = 'overdue'; filterDueDateName = 'Overdue'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 text-red-600 font-medium">Overdue</button>
                                <button @click="filterDueDate = 'completed'; filterDueDateName = 'Completed'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 text-emerald-600 font-medium">Completed</button>
                            </div>
                        </div>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false" type="button"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-750 border border-gray-200/90 dark:border-gray-700 rounded-xl text-xs font-medium text-gray-700 dark:text-gray-300 shadow-2xs transition">
                                <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                                <span>Label: <strong class="font-semibold" x-text="filterLabelName || 'All'"></strong></span>
                                <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            <div x-show="open" x-cloak class="absolute right-0 top-full mt-1.5 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-1.5 z-40 space-y-0.5 max-h-60 overflow-y-auto">
                                <button @click="filterLabel = ''; filterLabelName = 'All'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700" :class="filterLabel === '' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">All Labels</button>
                                @foreach($projectLabels as $lbl)
                                <button @click="filterLabel = '{{ $lbl->id }}'; filterLabelName = '{{ addslashes($lbl->name) }}'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-1.5" :class="filterLabel === '{{ $lbl->id }}' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" @click.away="open = false" type="button"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-750 border border-gray-200/90 dark:border-gray-700 rounded-xl text-xs font-medium text-gray-700 dark:text-gray-300 shadow-2xs transition">
                                <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6H9.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9"/></svg>
                                <span>Priority: <strong class="font-semibold" x-text="filterPriorityName || 'All'"></strong></span>
                                <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            <div x-show="open" x-cloak class="absolute right-0 top-full mt-1.5 w-40 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-1.5 z-40 space-y-0.5">
                                <button @click="filterPriority = ''; filterPriorityName = 'All'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700" :class="filterPriority === '' ? 'text-blue-600 font-bold' : 'text-gray-700 dark:text-gray-200'">All</button>
                                <button @click="filterPriority = 'urgent'; filterPriorityName = 'Urgent'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 text-rose-600 font-semibold">Urgent</button>
                                <button @click="filterPriority = 'high'; filterPriorityName = 'Important'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 text-amber-600 font-semibold">Important</button>
                                <button @click="filterPriority = 'medium'; filterPriorityName = 'Medium'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 text-blue-600 font-semibold">Medium</button>
                                <button @click="filterPriority = 'low'; filterPriorityName = 'Low'; open = false; applyFilters()" class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-500 font-semibold">Low</button>
                            </div>
                        </div>
                ...
                ============================================================ --}}
                @if($columns->isEmpty())
                <div class="bg-white dark:bg-gray-850 rounded-3xl border border-gray-200/90 dark:border-gray-700/80 p-8 sm:p-12 text-center shadow-2xs space-y-6">
                    {{-- Illustration Blueprint Box --}}
                    <div class="relative w-20 h-20 mx-auto rounded-3xl bg-blue-50 dark:bg-blue-950/60 border-2 border-blue-200/70 dark:border-blue-800/80 flex items-center justify-center text-blue-600 dark:text-blue-400 shadow-sm">
                        <svg class="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2H4V5zm0 4h16v10a1 1 0 01-1 1H5a1 1 0 01-1-1V9z" />
                        ...
                        <span class="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-blue-600 ring-4 ring-white dark:ring-gray-850"></span>
                    </div>
                    ...
                    <div class="space-y-2 max-w-md mx-auto">
                        ...
                        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                            Organize your sprint workflow by creating custom stages or instantly bootstrap with standard agile buckets.
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-center gap-3 flex-wrap pt-2">
                        ...
                        <button type="button"
                            ...
                            <span x-text="isBootstrapping ? 'Creating Buckets...' : 'Use Default Buckets'">Use Default Buckets</span>
                        </button>
                        <button type="button"
                    ...
                    <div class="pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-center gap-2 text-xs flex-wrap text-gray-400">
                        <span class="font-bold tracking-wider text-[10px] uppercase text-gray-400">AUTO-GENERATES:</span>
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold text-[11px]">
                            <span class="w-1.5 h-1.5 rounded-full bg-slate-400"></span> To Do
                        </span>
                        <span>&rarr;</span>
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 font-semibold text-[11px]">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span> IN PROGRESS
                        </span>
                        <span>&rarr;</span>
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 font-semibold text-[11px]">
                            <span class="w-1.5 h-1.5 rounded-full bg-purple-500"></span> REVIEW
                        </span>
                        <span>&rarr;</span>
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 font-semibold text-[11px]">
                    ...
                    <div id="project-detail-kanban-columns"
                            ...
                            <div class="kanban-column-header flex items-center justify-between gap-2 px-1 shrink-0 cursor-grab active:cursor-grabbing">
                                @if(in_array($slug, ['todo', 'to_do', 'backlog']) && $col->color === 'slate')
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        <h3 class="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-100 truncate">{{ $col->name }}</h3>
                                        <span class="column-counter text-[11px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full shadow-2xs">{{ $colTasks->count() }}</span>
                                    </div>
                                @else
                                    <div class="flex items-center gap-2 min-w-0">
                                        <span class="column-badge inline-flex items-center gap-1.5 {{ $pillBg }} text-white font-bold text-xs uppercase tracking-wider px-3 py-1 rounded-full shadow-2xs truncate">
                                            {!! $col->renderIcon('w-3.5 h-3.5 text-white shrink-0') !!}
                                            <span class="truncate">{{ $col->name }}</span>
                                            <span class="column-counter bg-white/25 text-white text-[10.5px] px-1.5 py-0.2 rounded-full font-bold">{{ $colTasks->count() }}</span>
                                        </span>
                                    </div>
                                @endif
                                ...
                                <div class="relative shrink-0" x-data="{ open: false }">
                                    ...
                                    <div x-show="open" x-cloak
                                        ...
                                        <div class="px-2.5 py-2 flex items-center justify-between gap-2 border-b border-gray-100 dark:border-gray-800 pb-2 mb-1">
                                            ...
                                            <span class="text-[10.5px] font-bold px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-600 dark:text-gray-300 shrink-0">{{ $colTasks->count() }}</span>
                                        </div>
                                        ...
                                        <button type="button"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-xs font-bold text-gray-800 dark:text-gray-200 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Edit Bucket</p>
                                        ...
                                        <button type="button"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-xs font-bold text-gray-800 dark:text-gray-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">Add Task to Bucket</p>
                                                <p class="text-[10px] text-gray-400 truncate">Quick create a task in this bucket</p>
                                            </div>
                                        </button>
                                        ...
                                        <button type="button"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="text-[10px] text-red-400/90 dark:text-red-400/80 truncate">Move tasks or delete completely</p>
                                            </div>
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="add-bucket-column-wrapper w-80 shrink-0">
                            <button type="button"
                                ...
                                <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 group-hover:bg-blue-600 group-hover:text-white text-gray-400 flex items-center justify-center transition-colors shadow-2xs">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                                </div>
                                <div>
                                    <span class="text-xs font-bold text-gray-700 dark:text-gray-300 group-hover:text-blue-600 transition-colors block">Add Another Bucket</span>
                                    <span class="text-[11px] text-gray-400 block mt-0.5">e.g. Backlog, Blocked, Testing</span>
                                </div>
                            </button>
                        </div>
                        @endif
                ...
                <div x-show="taskView === 'list'" x-cloak class="pt-1 w-full max-w-full min-w-0">
                    <div class="bg-white dark:bg-gray-850 rounded-2xl border border-gray-200/80 dark:border-gray-700 p-6 shadow-2xs space-y-6">
                        {{-- Table Header Row --}}
                        <div class="grid grid-cols-12 gap-2 text-[12px] font-bold text-gray-400 dark:text-gray-500 pb-3 border-b border-gray-100 dark:border-gray-800 px-3 items-center">
                        ...
                        <div class="space-y-2" x-data="{ accordionOpen: true }">
                            ...
                            <div class="flex items-center gap-2.5 cursor-pointer pt-1" @click="accordionOpen = !accordionOpen">
                                <svg class="w-4 h-4 text-gray-400 transition-transform duration-200" :class="accordionOpen ? '' : '-rotate-90'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/>
                                </svg>

                                @if(in_array($slug, ['todo', 'to_do', 'backlog']) && $col->color === 'slate')
                                    <span class="column-badge inline-flex items-center gap-1.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 font-bold px-3 py-0.5 rounded-full text-xs shadow-2xs">
                                        <svg class="w-3.5 h-3.5 text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                                        <span>{{ $col->name }}</span>
                                    </span>
                                @else
                                    <span class="column-badge inline-flex items-center gap-1.5 {{ $pillBg }} text-white font-bold px-3 py-0.5 rounded-full text-xs shadow-2xs">
                            ...
                            <div x-show="accordionOpen" class="space-y-1 pt-1">
                                <div id="list-tasks-column-{{ $col->id }}"
                                    ...
                                    <div class="list-task-row grid grid-cols-12 gap-2 py-2.5 px-3 hover:bg-gray-50/90 dark:hover:bg-gray-800/60 rounded-xl transition-all items-center group cursor-pointer border border-transparent hover:border-gray-100 dark:hover:border-gray-750"
                                        ...
                                        <div class="col-span-4 flex items-center gap-2.5 min-w-0 pr-2">
                                            <span class="list-drag-handle cursor-grab active:cursor-grabbing text-gray-300 group-hover:text-gray-500 p-0.5 shrink-0" @click.stop>
                                            ...
                                            <span class="text-xs font-bold text-gray-800 dark:text-gray-100 group-hover:text-blue-600 truncate {{ $isDone ? 'line-through text-gray-400 dark:text-gray-500' : '' }}">
                                                {{ $task->title }}
                                            </span>
                                        </div>

                                        {{-- 2. Priority (col-span-1) --}}
                                        <div class="col-span-1 list-priority-container flex items-center gap-1.5 text-xs font-bold {{ $plc['class'] }}">
                                        ...
                                        <div class="col-span-1 list-assignee-container flex items-center">
                                            @if($allMembers->isNotEmpty())
                                                ...
                                                @if($assignee->avatar)
                                                    <img src="{{ Storage::url($assignee->avatar) }}" alt="{{ $assignee->name }}" title="{{ $assignee->name }}" class="w-6 h-6 rounded-full object-cover shadow-2xs">
                                                @else
                                                    <div class="w-6 h-6 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center text-[10px] font-bold shadow-2xs" title="{{ $assignee->name }}">
                                        ...
                                        <div class="col-span-2 list-labels-container flex items-center gap-1.5 flex-wrap">
                                            @if($task->labels->isNotEmpty())
                                                ...
                                                @endphp
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10.5px] font-bold {{ $lStyle }} truncate max-w-[120px]">
                                                    {{ $lbl->name }}
                                                </span>
                                                @endforeach
                                        ...
                                        <div class="col-span-1 list-checklist-container flex items-center">
                                            @if($totalCount > 0)
                                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11px] font-semibold {{ $pctChecklist === 100 ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400' : 'bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-400' }}">
                                        ...
                                        <div class="col-span-1 list-details-container flex items-center justify-end gap-2 text-xs">
                                            {{-- Comments count --}}
                                            <span class="inline-flex items-center gap-1 text-[11px] {{ $task->comments_count > 0 ? 'text-gray-500 dark:text-gray-400 font-medium' : 'text-gray-300 dark:text-gray-600' }}" title="{{ $task->comments_count }} comments">
                                            ...
                                            <span class="inline-flex items-center gap-1 text-[11px] text-gray-500 dark:text-gray-400 font-medium" title="{{ $task->attachments_count }} attachments">
                                ...
                                <div class="flex items-center justify-between px-3 pt-2 text-xs">
                                    ...
                                    <span class="text-[11px] font-semibold text-gray-400">Count {{ $colTasks->count() }}</span>
                                </div>
                            </div>
                        </div>
                        @endforeach
            ...
            <div x-show="showBucketModal" x-cloak
                ...
                <div class="bg-white dark:bg-gray-850 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-2xl w-full max-w-md overflow-hidden transform transition-all"
                    ...
                    {{-- Modal Header --}}
                    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-2xl bg-blue-50 dark:bg-blue-950/60 border border-blue-200/60 flex items-center justify-center text-blue-600 dark:text-blue-400">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2H4V5zm0 4h16v10a1 1 0 01-1 1H5a1 1 0 01-1-1V9z"/></svg>
                            </div>
                            <div>
                                <h3 class="text-base font-black text-gray-900 dark:text-white" x-text="bucketModalMode === 'edit' ? 'Edit Bucket' : 'Create New Bucket'">Create New Bucket</h3>
                                <p class="text-[11px] text-gray-400">Define pipeline phase and cadence rules</p>
                            </div>
                        </div>
                        <button type="button" @click="showBucketModal = false" class="text-gray-400 hover:text-gray-600 p-1.5 rounded-lg hover:bg-gray-100 cursor-pointer">
                    ...
                    <form @submit.prevent="saveBucket()" class="p-6 space-y-5">
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold text-gray-700 dark:text-gray-300">Bucket Title / Name</label>
                            <div class="relative">
                                <input type="text"
                                       x-model="bucketForm.name"
                                ...
                                <span x-show="bucketForm.name.trim().length > 0" class="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-500">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-7 gap-2">
                                <template x-for="ic in ['clipboard', 'flame', 'shield', 'check', 'bug', 'rocket', 'window']" :key="ic">
                                    <button type="button"
                                            @click="bucketForm.icon = ic"
                                        ...
                                        <template x-if="ic === 'bug'">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0a8 8 0 01-8 8m0 0a8 8 0 01-8-8m8-8a8 8 0 018 8m-8-8a8 8 0 00-8 8"/></svg>
                                        </template>
                                        <template x-if="ic === 'rocket'">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                        </template>
                                        <template x-if="ic === 'window'">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2H4V5zm0 4h16v10a1 1 0 01-1 1H5a1 1 0 01-1-1V9z"/></svg>
                                        </template>
                                    </button>
                                </template>
                            </div>
                        ...
                        {{-- Theme Accent Color --}}
                        <div class="space-y-2">
                            <label class="block text-xs font-bold text-gray-700 dark:text-gray-300">Theme Accent Color</label>
                            <div class="flex items-center gap-3">
                                @php
                                    $paletteOptions = [
                                ...
                                <button type="button"
                                        @click="bucketForm.color = '{{ $pCol['key'] }}'"
                        ...
                        <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                            ...
                            <button type="submit"
                                    :disabled="isSavingBucket || !bucketForm.name.trim()"
                                ...
                                <span x-text="isSavingBucket ? 'Saving...' : (bucketModalMode === 'edit' ? 'Save Changes' : 'Create Bucket')">Create Bucket</span>
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="bg-white dark:bg-gray-850 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-2xl w-full max-w-md overflow-hidden transform transition-all"
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex items-center gap-3.5">
                            <div class="w-12 h-12 rounded-2xl bg-red-50 dark:bg-red-950/60 border border-red-200 dark:border-red-800/80 flex items-center justify-center text-red-600 dark:text-red-400 shrink-0">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Bucket: <strong class="text-gray-800 dark:text-gray-200" x-text="deleteBucketData.name"></strong></p>
                            </div>
                        </div>

                        {{-- Warning text & Task handling --}}
                        <template x-if="deleteBucketData.taskCount > 0">
                            <div class="p-3.5 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/70 rounded-2xl space-y-2.5 text-xs text-amber-900 dark:text-amber-300">
                                <p class="font-semibold flex items-center gap-1.5">
                                    <svg class="w-4 h-4 text-amber-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    <span>This bucket contains <strong x-text="deleteBucketData.taskCount"></strong> task(s).</span>
                                </p>

                                <div class="space-y-2 pt-1">
                                    <label class="block text-[11px] font-bold text-gray-700 dark:text-gray-300">Choose what to do with the tasks:</label>

                                    <div class="space-y-2">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" value="move" x-model="deleteBucketData.action" class="text-blue-600 focus:ring-blue-500">
                                            <span class="text-xs text-gray-700 dark:text-gray-300 font-medium">Move tasks to another bucket:</span>
                                        </label>

                                        <div x-show="deleteBucketData.action === 'move'" class="pl-6 pt-0.5">
                                            <select x-model="deleteBucketData.moveToColumnId" class="w-full text-xs p-2.5 rounded-xl border border-gray-300 dark:border-gray-650 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-blue-500/20">
                                                @foreach($columns as $otherCol)
                                                <option value="{{ $otherCol->id }}" x-show="deleteBucketData.id !== {{ $otherCol->id }}">{{ $otherCol->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <label class="flex items-center gap-2 cursor-pointer pt-1">
                                            <input type="radio" value="delete_all" x-model="deleteBucketData.action" class="text-red-600 focus:ring-red-500">
                                            <span class="text-xs text-red-600 dark:text-red-400 font-medium">Permanently delete all tasks in this bucket</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                    ...
                    {{-- Footer Actions --}}
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/60 border-t border-gray-100 dark:border-gray-800 flex items-center justify-end gap-2.5">
