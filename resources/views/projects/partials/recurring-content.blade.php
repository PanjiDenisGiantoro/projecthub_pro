<div x-data="{ showRecurringForm: false, recurringFreq: 'daily' }">
    <div class="flex justify-between items-center mb-4">
        <p class="text-sm text-gray-500">{{ $recurringDefinitions->total() }} definisi terdaftar</p>
        @if(!auth()->user()->hasRole('client'))
        <button @click="showRecurringForm = !showRecurringForm"
                class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            <span x-text="showRecurringForm ? 'Batal' : 'Tambah'"></span>
        </button>
        @endif
    </div>
    ...
    <div x-show="showRecurringForm" x-cloak class="bg-white rounded-xl border border-blue-200 p-5 mb-5">
        ...
        <form method="POST" action="{{ route('recurring.store', $project) }}" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="frequency" required x-model="recurringFreq"
            ...
            <div x-show="recurringFreq==='weekly' || recurringFreq==='biweekly'" x-cloak>
                <label class="block text-xs font-medium text-gray-600 mb-1">Hari (untuk Mingguan/2 Minggu sekali)</label>
                <select name="day_of_week" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div x-show="recurringFreq==='monthly'" x-cloak>
                <label class="block text-xs font-medium text-gray-600 mb-1">Tanggal (untuk Bulanan)</label>
                <input type="number" name="day_of_month" value="1" min="1" max="28" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <select name="milestone_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach($recurringMilestones as $m)
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach($recurringDefinitions as $def)
        ...
        @if($recurringDefinitions->hasPages())
        <div class="px-4 py-3 border-t border-gray-100">{{ $recurringDefinitions->links() }}</div>
        @endif
        @endif
    </div>
</div>
