@php $activeSprint = $sprints->firstWhere('status', 'active'); @endphp
<div x-data="{ showSprintForm: false }">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-base font-semibold text-gray-900">Sprints ({{ $sprintList->total() }})</h2>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('board-columns.index', $project) }}"
               class="inline-flex items-center gap-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 4.5v15m6-15v15m-10.875 0h15.75c.621 0 1.125-.504 1.125-1.125V5.625c0-.621-.504-1.125-1.125-1.125H4.125C3.504 4.5 3 5.004 3 5.625v12.75c0 .621.504 1.125 1.125 1.125Z"/></svg>
                Kelola Kolom
            </a>
            <button @click="showSprintForm = !showSprintForm"
                    class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                <span x-text="showSprintForm ? 'Batal' : 'Sprint Baru'"></span>
            </button>
        </div>
        @endif
    </div>
    ...
    {{-- New Sprint Form --}}
    @if(!auth()->user()->hasRole('client'))
    <div x-show="showSprintForm" x-cloak class="bg-white rounded-xl border border-blue-200 p-5 mb-5">
    ...
    @if($sprintList->hasPages())
    <div class="mb-6">{{ $sprintList->links() }}</div>
    @else
    <div class="mb-6"></div>
    @endif
